            ...
            if request_id.is_empty() {
                ...
                match ctx.approval_manager.confirm_non_cli_pending_request(
                    ...
                    Ok(req) => {
                        ...
                        format!(
                            "Approved pending request `{request_id}` for `{}`.\nThe waiting supervised tool execution can now continue.",
                            approval_target_label(&tool_name)
                        )
                    }
                    Err(PendingApprovalError::NotFound) => {
                        runtime_trace::record_event(
                    ...
                    Err(PendingApprovalError::RequesterMismatch) => {
                        ...
                        format!(
                            "Pending approval request `{request_id}` can only be resolved by the same sender in the same chat/channel that created it."
                        )
                    }
                }
            }
            ...
            if request_id.is_empty() {
                ...
                match ctx.approval_manager.reject_non_cli_pending_request(
                    ...
                    Ok(req) => {
                        ...
                        format!(
                            "Denied pending request `{request_id}` for `{}`.\nThe waiting supervised tool execution will be rejected.",
                            approval_target_label(&tool_name)
                        )
                    }
                    Err(PendingApprovalError::NotFound) => {
